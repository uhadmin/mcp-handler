...
var chunk5DCZO5D7_js = require('./chunk-5DCZO5D7.js');
...
function createEvent(event) {
  return chunk5DCZO5D7_js.__spreadProps(chunk5DCZO5D7_js.__spreadValues({}, event), {
    timestamp: Date.now()
  });
}
...
var EventEmittingResponse = class extends http.ServerResponse {
  ...
  emitEvent(event) {
    if (this.onEvent) {
      this.onEvent(
        createEvent(chunk5DCZO5D7_js.__spreadProps(chunk5DCZO5D7_js.__spreadValues({}, event), {
...
function initializeRedis(_0) {
  return chunk5DCZO5D7_js.__async(this, arguments, function* ({
    redisUrl,
    logger
  }) {
...
function initializeMcpApiHandler(initializeServer, serverOptions = {}, config = {
  ...
  } = _a, mcpServerOptions = chunk5DCZO5D7_js.__objRest(_a, [
    "serverInfo"
  ]);
  ...
  return function mcpApiHandler(req, res) {
    return chunk5DCZO5D7_js.__async(this, null, function* () {
      var _a2, _b, _c;
      ...
      if (url.pathname === streamableHttpEndpoint) {
        ...
        if (req.method === "POST") {
          ...
          const statelessServer = new mcp_js.McpServer(serverInfo, mcpServerOptions);
        ...
        let logs = [];
        const cleanup = (reason) => chunk5DCZO5D7_js.__async(null, null, function* () {
        ...
        try {
          ...
          handleMessage = (message) => chunk5DCZO5D7_js.__async(null, null, function* () {
        ...
        let isCleanedUp = false;
        const cleanup = () => chunk5DCZO5D7_js.__async(null, null, function* () {
          if (isCleanedUp) return;
          isCleanedUp = true;
          if (timeout) {
            clearTimeout(timeout);
        ...
        const sendResponse = (status, body2) => chunk5DCZO5D7_js.__async(null, null, function* () {
          if (!hasResponded) {
            hasResponded = true;
        ...
        const handleResponse = (message) => chunk5DCZO5D7_js.__async(null, null, function* () {
          try {
        ...
        try {
          ...
          timeout = setTimeout(() => chunk5DCZO5D7_js.__async(null, null, function* () {
            yield sendResponse(408, "Request timed out");
          }), 10 * 1e3);
          res.on("close", () => chunk5DCZO5D7_js.__async(null, null, function* () {
            if (!hasResponded) {
              hasResponded = true;
              yield cleanup();
            }
          }));
          res.on("error", (error) => chunk5DCZO5D7_js.__async(null, null, function* () {
...
function createServerResponseAdapter(signal, fn) {
  ...
  return new Promise((resolve) => {
    ...
    void (() => chunk5DCZO5D7_js.__async(null, null, function* () {
...
function withMcpAuth(handler, verifyToken, {
  ...
  return (req) => chunk5DCZO5D7_js.__async(null, null, function* () {
...
function protectedResourceHandler({
  ...
  return (req) => {
    ...
    return new Response(JSON.stringify(metadata), {
      headers: chunk5DCZO5D7_js.__spreadProps(chunk5DCZO5D7_js.__spreadValues({}, corsHeaders), {
